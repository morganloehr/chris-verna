...
if ( ( $thankyou = get_query_var( 'edu-thankyou' ) ) ) {
	...
	?>
	<h3><?php esc_html_e( 'Payment Summary', 'training' ); ?></h3>

	<dl id="payment-details" class="edu-dl">
		<dt class="payment-id"><?php esc_html_e( 'Payment', 'training' ); ?></dt>
		<dd><?php echo intval( $payment->ID ); ?></dd>

		<dt class="payment-date"><?php esc_html_e( 'Date', 'training' ); ?></dt>
		<dd><?php echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( $payment->payment_date ) ) ); ?></dd>

		<dt class="payment-status"><?php esc_html_e( 'Payment Status', 'training' ); ?></dt>
		<dd>
			<?php
	...
	<table class="edu-payment-table">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Item', 'training' ); ?></th>
				<th><?php esc_html_e( 'Price', 'training' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<tr>
	...
	<dl class="edu-payment-summary edu-dl">
		<?php
			if ( $payment->tax > 0.0 ) {
				echo '<dt class="payment-subtotal">' . esc_html__( 'Subtotal', 'training' ) .'</dt><dd>' . ib_edu_format_price( $payment->amount - $payment->tax, false ) . '</dd>';
		...
		<dt class="payment-total"><?php esc_html_e( 'Total', 'training' ); ?></dt>
	...
	if ( $payments_page ) {
		echo '<p>' . sprintf( esc_html__( 'Go to %s page', 'training' ), '<a href="' . esc_url( get_permalink( $payments_page->ID ) ) . '">'
	...
	?>
		<form id="ib-edu-payment-form" class="ib-edu-form" action="<?php echo esc_url( $form_action ); ?>" method="post">
			...
			<fieldset>
				<legend><?php esc_html_e( 'Payment Information', 'training' ); ?></legend>
				...
				<?php if ( $args['price'] && ! empty( $gateways ) ) : ?>
					<div class="ib-edu-form-field<?php if ( in_array( 'empty_payment_method', $error_codes ) ) echo ' error'; ?>">
						<label><?php esc_html_e( 'Payment Method', 'training' ); ?> <span class="required">*</span></label>
										...
										<li>
											<label>
												<input
													...
													<?php echo esc_attr($checked) ?>> <?php echo esc_html( $gateway->get_title() ); ?>
											</label>
										</li>
										<?php
									}
			...
			<div class="ib-edu-form-actions">
				<button type="submit" class="btn btn-success radius-4x"><?php esc_html_e( 'Continue', 'training' ) ?></button>
			</div>
		</form>
	<?php
}
