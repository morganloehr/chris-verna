...
if(  class_exists("WP_Customize_Control") ){
    ...
    class WPO_CustomizeProfile_DropDown extends WP_Customize_Control{
         public function render_content(){
            ...
            $profiles = training_wpo_cst_css_profiles();
...
!important: All theme profiles are stored in folder YOURTHEME/css/customze, it need permission 0755 to put files inside', 'training').'
            </div>' .'<div><a href="'.admin_url('themes.php?page=wpo_livethemeedit').'">'.esc_html__('Create Profile Now', 'training').'</a></div>';
            printf( 
    ...
    class WPO_Layout_DropDown extends WP_Customize_Control{
        ...
        public function render_content(){
            ...
            $layouts = array(
                'fullwidth' => esc_html__('Fullwidth', 'training'),
                'leftmain' => esc_html__('Left - Main Sidebar', 'training'),
                'mainright' => esc_html__('Main - Right Sidebar', 'training'),
                'leftmainright' => esc_html__('Left - Main - Right Sidebar', 'training'),
            ...
            <div class="page-layout">
                <img  src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI.'1col.png'; ?>" class="layout" data-value="fullwidth" alt="<?php esc_html_e( "Layout Main Content Column Only", 'training');?>">
                <img src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI.'2cl.png'; ?>" class="layout"data-value="leftmain" alt="<?php esc_html_e( "Layout Left  + Main Content Column", 'training');?>">
                <img src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI.'2cr.png'; ?>" class="layout" data-value="mainright" alt="<?php esc_html_e( "Layout Main Content and Right Column", 'training');?>">
                <img src="<?php echo WPO_FRAMEWORK_ADMIN_IMAGE_URI.'3c.png'; ?>" class="layout" data-value="leftmainright" alt="<?php esc_html_e( "Layout Left  + Main Content and Right Column", 'training');?>">
        ...
        public function render_content() {
            $dropdown = wp_dropdown_categories( 
                array( 
                    ...
                    'echo'             => 0,
                    'hide_empty'       => false,
                    'show_option_none' => '&mdash; ' . esc_html__('Select', 'training') . ' &mdash;',
                    'hide_if_empty'    => false,
                    'selected'         => $this->value(),
                )
            );
...
if ( class_exists( 'WP_Customize_Control' ) ) {
    ...
    class WPOpalTextAreaControl extends WP_Customize_Control {
        ...
        public function __construct( $manager, $id, $args = array() ) {
            $this->statuses = array( '' => esc_html__( 'Default', 'training' ) );
