...
<?php if ( $questions ) : ?>
	<?php
		...
		if ( $message ) {
			switch ( $message ) {
				case 'empty-answers':
					echo '<div class="ib-edu-message error">' . esc_html__( 'Please answer all questions before submitting the quiz.', 'training' ) . '</div>';
					break;

				case 'quiz-submitted':
					echo '<div class="ib-edu-message success">' . esc_html__( 'Thank you. The quiz has been accepted.', 'training' ) . '</div>';
					break;
			}
		}
	...
	<section id="ib-edu-quiz" class="<?php echo ( $quiz_submitted ) ? 'ib-edu-quiz-complete' : 'ib-edu-quiz-inprogress'; ?>">
		<?php if ( $quiz_submitted ) : ?>
			...
			<section class="ib-edu-quiz-grade">
				<h3><?php esc_html_e( 'Quiz Grade', 'training' ); ?></h3>
				<p class="grade">
					<?php
		...
		?>
		<h3 class="ib-edu-quiz-title"><?php esc_html_e( 'Quiz', 'training' ); ?></h3>
		...
		<form id="ib-edu-quiz-form" class="ib-edu-form" method="post" action="<?php echo esc_url( ib_edu_get_endpoint_url( 'edu-action', 'submit-quiz', get_permalink() ) ); ?>">
			...
			<?php if ( ! $quiz_submitted ) : ?>
				<div class="ib-edu-buttons">
					<button class="ib-edu-button" type="submit"><?php esc_html_e( 'Submit', 'training' ); ?></button>
				</div>
			<?php endif; ?>
		</form>
	</section>
