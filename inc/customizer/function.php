...
function training_wpo_cst_skins(){
...
function training_wpo_cst_headerlayouts(){
    $path = WPO_THEME_DIR.'/header-*.php';
    $files = glob($path  );
    $headers = array( '' => esc_html__('Default', 'training' ) );
    if(count($files)>0){
        foreach ($files as $key => $file) {
...
function training_wpo_cst_css_profiles(){
    $path = WPO_THEME_DIR.'/customize/assets/*.css';
    $files = glob($path  );
    $skins = array( 'nouse' => esc_html__('No Use', 'training') );
...
function training_wpo_cst_fonts_list(){
    $font_faces = array(
        "'Arvo', serif" => "Arvo",
...
function training_wpo_get_menuanimation(){
    return array(
        ''        => 'None',
        'slide'   => 'Slide',
        'zoom'    => 'Zoom',
...
function training_wpo_get_footerbuilder_profiles() {
...
function training_wpo_get_styles( $path, $file_name){
    //$path = WPO_THEME_DIR.'/temheader-*.php';
    $files = glob($path  );
    $styles = array( '' => esc_html__('Default', 'training' ) );
    if(count($files)>0){
        foreach ($files as $key => $file) {
            $style = str_replace( $file_name, '', str_replace( '.php', '', basename($file) ) );
            $styles[$style] = esc_html__( ucfirst( $style ), 'training' );
        }
    }
...
add_action( 'customize_register', 'training_wpo_cst_customize_register' );


function training_wpo_cst_customize_preview_assets() { 
...
add_action( 'customize_controls_enqueue_scripts', 'training_wpo_cst_customize_preview_assets' );