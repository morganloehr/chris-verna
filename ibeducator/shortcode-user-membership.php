...
if ( ! $user_membership ) {
	echo '<p>' . esc_html__( 'Your account is not connected to a membership.' , 'training' ) . '</p>';
	return;
}
...
switch ( $action ) {
	default:
		...
		<table class="ib-edu-membership">
			<tbody>
				<tr>
					<th style="width:30%;"><?php esc_html_e( 'Membership Level', 'training' ); ?></th>
					<td>
						...
						<div>
							<?php
								echo ib_edu_purchase_link( array(
									...
									'text'      => esc_html__( 'Extend', 'training' ),
								) );
							?>
						</div>
					</td>
				...
				<tr>
					<th><?php esc_html_e( 'Status', 'training' ); ?></th>
					<td>
						<?php
							...
							if ( 1 == ib_edu_get_option( 'pause_memberships', 'memberships' ) ) {
								if ( 'active' == $user_membership['status'] ) {
									...
									echo ' <span class="pause-membership"><a class="ib-edu-button" href="' . esc_url( $pause_url )
										. '">' . esc_html__( 'Pause', 'training' ) . '</a></span>';
									...
									echo ' <span class="pause-membership"><a class="ib-edu-button" href="' . esc_url( $resume_url )
										. '">' . esc_html__( 'Resume', 'training' ) . '</a></span>';
								}
							}
						?>
						</div>
				...
				<tr>
					<th><?php esc_html_e( 'Expiration Date', 'training' ); ?></th>
					<td>
						<?php
							if ( ! $user_membership['expiration'] ) {
								esc_html_e( 'None', 'training' );
							} else {
