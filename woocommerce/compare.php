...
add_action( 'wp_enqueue_scripts', 'training_enqueue_script_compare' );
...
<!-- END HEAD -->
<!-- START BODY -->
<body <?php body_class('training') ?>>

<h1>
    <?php echo get_option( 'yith_woocompare_table_text' ) ?>
    <?php if ( ! $is_iframe ) : ?><a class="close" href="#"><?php esc_html_e( 'Close window [X]', 'training' ) ?></a><?php endif; ?>
</h1>
...
<table class="table table-bordered compare-list" cellpadding="0" cellspacing="0"<?php if ( empty( $products ) ) echo ' style="width:100%"' ?>>
    ...
    <?php if ( empty( $products ) ) : ?>
        ...
        <tr class="no-products">
            <td><?php esc_html_e( 'No products added in the compare table.', 'training' ) ?></td>
        </tr>
        ...
        <tr class="remove">
            ...
            <?php foreach( $products as $i => $product ) : $product_class = ( $i % 2 == 0 ? 'odd' : 'even' ) . ' product_' . $product->id ?>
                <td class="<?php echo esc_attr($product_class); ?>">
                    <a href="<?php echo add_query_arg( 'redirect', 'view', $this->remove_product_url( $product->id ) ) ?>" data-product_id="<?php echo esc_html($product->id); ?>"><?php esc_html_e( 'Remove', 'training' ) ?> <span class="remove">x</span></a>
                </td>
            <?php endforeach ?>
        </tr>
