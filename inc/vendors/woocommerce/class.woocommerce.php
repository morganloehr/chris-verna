...
if(class_exists('training')){
	...
	class Training_WPO_Woocommerce{
		
		public static function getInstance(){
			static $_instance;
			if( !$_instance ){
				$_instance = new Training_WPO_Woocommerce();
			}
			return $_instance;
		}
		...
		public function __construct(){
			...
			add_action( 'wpo_button_display',array('Training_WPO_Woocommerce','renderButton'),20 );


 

			if( training_wpo_theme_options( 'is-swap-effect',true ) ){
		...
		public function wc_product_per_page($cols){
			return training_wpo_theme_options('woo-number-page',12);
		}

		public function wc_loop_shop_columns(){
			return training_wpo_theme_options('wc_itemsrow',3);
		}
		...
		public function woocommerce_template_loop_product_thumbnail(){
			...
			if(has_post_thumbnail()){
				...
				$output .= '<img src="'.woocommerce_placeholder_img_src().'" alt="'.esc_html__('Placeholder' , 'training').'" class="'.$class.'" width="'.$placeholder_width.'" height="'.$placeholder_height.'" />';
			}
			echo trim($output);
		}
	...
	Training_WPO_Woocommerce::getInstance();
}