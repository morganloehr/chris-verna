...
<p class="entry-link">
    <a  href="<?php echo esc_url($post->link); ?>" class="vc_read_more btn btn-outline"
        title="<?php echo esc_attr( sprintf( esc_html__( 'Permalink to %s', 'training' ), $post->title_attribute ) ); ?>"<?php echo trim($this->link_target) ?>><?php esc_html_e( 'Read more', 'training' ) ?></a>
</p>
<?php endif; ?>